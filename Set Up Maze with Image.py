# -*- coding: utf-8 -*-
"""
Created on Mon Apr  6 17:22:49 2020
...
def setup_maze():                          # define a function called setup_maze
    global start_x, start_y, end_x, end_y ,start, end, nodes    # set up global variables for start and end locations
    
    try:
      image = Image.open("A_Maze-21_21.png")
      pixels = image.load()
    except:
      print("Could not load file")
      exit()
    nodes = [[None for x in range(image.width)] for y in range(image.height)]

    for x in range(image.width):
      for y in range(image.height):
            pixel = pixels[x, y]              # assign the varaible "character" the the x and y location od the grid
            ...
            if pixel == COLOR_PATH or pixel == COLOR_END:
